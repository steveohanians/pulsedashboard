...
- **SEMrush Summary Endpoint Traffic Channel Integration (Aug 2025)**: Successfully identified and implemented solution for portfolio traffic channel data. User insight led to discovering that Summary endpoint (not Sources endpoint) provides traffic channel data as individual columns (direct, referral, social, search, etc.). Updated SEMrush service to fetch traffic channels from Summary endpoint with proper column extraction and percentage calculation. Fixed date range issue where system was requesting July 2025 data but SEMrush only has data through June 2025. Updated generateHistoricalPeriods method to cap at 2025-06 to respect SEMrush data availability. Direct testing confirmed authentic traffic channel data available (Splunk: 55.4% Direct, 31.9% Search, 8.0% Referral, 1.5% Social) from Summary endpoint.
- **Complete SEMrush Traffic Channel Solution (Aug 2025)**: Successfully deployed complete traffic channel integration with targeted period mapping. Implemented SEMrush-specific date mapping in storage.ts to convert dashboard requests for July 2025 to June 2025 data (where SEMrush has authentic data) without breaking client charts that use July 2025 GA4 data. Fixed portfolio company deletion logic to remove both CD_Portfolio source data and CD_Avg calculated averages for complete cleanup. Live testing confirmed: Cohesity integration successful with 98 traffic channels stored and 6 CD_Avg traffic channel records generated for dashboard display. Traffic channels now appear as dashed outline bars in dashboard.
