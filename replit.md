...
- **AI Insights Persistence System**: Comprehensive database-backed insights with server-computed badge persistence, true transactional deletion, and period-compatible queries. Features server-side `hasContext` computation using dual-source checking (aiInsights.contextText + insightContexts table), DELETE endpoint returning `{ok: true, deleted: {insights: N, contexts: N}}` format, canonical YYYY-MM period handling with backward compatibility, and production-grade query key invalidation. Server-computed "With Context" badge implementation validates database state on GET `/api/ai-insights/:clientId?period=YYYY-MM` with `hasContext` persisting across reloads. Validated August 11, 2025 with comprehensive test suite confirming badge persistence, context-preferring reload behavior, server-truth badge rendering, and authentic delete operations.
- **AI Insights Loading State Resolution**: Fixed critical "Regenerating..." forever issue through centralized hook optimization and proper spinner logic. Implemented instrumented `useAIInsights` hook with canonical period normalization, aggressive fetching settings (staleTime: 0, refetchOnMount: "always"), and precise loading state management. Resolved infinite spinner by correcting `isRegenerating` logic to only trigger on server-reported "generating" status, not general data loading states. Added comprehensive console instrumentation for debugging load states. Single fetch per period with proper cache invalidation prevents burst requests and rate limiting. System properly distinguishes "pending" (show generate button) vs "generating" (show spinner) states. Validated August 12, 2025 with browser logs confirming single network requests and correct state transitions.
