...
- **Systematic Clear Insights Rebuild (Aug 2025)**: Completely rebuilt the clear insights functionality with proper separation of concerns. Eliminated complex cache invalidation issues by implementing systematic coordination between database, React Query cache, and UI state layers. Enhanced backend endpoint with structured responses, created dedicated functions for each layer (database clearing, cache invalidation, UI state reset), and removed nuclear fallback approaches like page reloads. Applied systematic cache invalidation patterns to competitor deletion as well. System now provides reliable, predictable clearing with proper error handling and no race conditions.
- **AI Insights Cache Invalidation Fix (Aug 2025)**: Resolved critical caching issue where newly generated AI insights would not appear without page refresh. Implemented targeted cache invalidation strategy replacing aggressive `queryClient.clear()` with surgical cache removal to preserve typewriter effects. Added backend performance cache clearing to all insight generation endpoints (/api/generate-metric-insight, /api/generate-metric-insight-with-context, /api/generate-comprehensive-insights). System now ensures fresh insights appear immediately with smooth typewriter animation and no page refreshes.
