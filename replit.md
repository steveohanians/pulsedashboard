...
- **Data Integrity**: Client data isolation verification, metric versioning, company validation and normalization utilities, robust company creation and deletion logic. Strict policy of never showing fake or sample data; empty states are preferred. Cross-type domain validation disabled - domains can be used across all company types (competitor, portfolio, benchmark, client) without restrictions.
