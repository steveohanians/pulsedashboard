...
- **AI Insights Rate Limiting & Empty State Architecture**: Implemented comprehensive solution to prevent rate limiting cascades when no insights exist. Created batch generation endpoint (`/api/insights/generate-batch`) that generates all 6 metrics simultaneously with 1-second delays to prevent OpenAI API limits. Added circuit breaker pattern with progressive backoff (30s, 60s, 120s) that blocks individual API calls when rate limits detected. Enhanced dashboard-level empty state detection to trigger batch generation instead of allowing 6 individual API calls. Individual metric components now respect circuit breaker state and won't make fallback API calls during batch generation or when rate limited. Eliminated rate limit storms that previously occurred when insights were cleared or missing (August 2025).
