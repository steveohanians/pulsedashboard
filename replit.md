...
- **Complete Sample Data Elimination (August 2025)**: Systematic removal of all sample data generation functionality to enforce authentic data only policy. Phase 1: Deleted entire `server/services/sampleData/` directory and 3 API endpoints. Phase 2: Eliminated remaining sample data files (`sampleDataGenerator.ts`, `sampleDataConfig.ts`, `channelDataGenerator.ts`), removed `/api/admin/sample-data-config` endpoint, cleaned up all sample data references and legacy comments. Phase 3: Database cleanup - deleted all benchmark companies (3), CD portfolio companies (7), and associated metric data, leaving only Demo Company with authentic GA4 data. System operates exclusively with authentic data sources with zero synthetic data capabilities.
- **CD Portfolio Form Simplification (August 2025)**: Removed description fields from Add Company to CD Portfolio modal and database storage. Updated frontend form in admin-panel.tsx to eliminate description textarea, modified database schema to drop description column, and cleaned up all related API references. Streamlined CD Portfolio company creation process to essential fields only (name, website, industry, business size).
...
- **Complete SEMrush Integration Package (August 2025)**: Comprehensive automated SEMrush integration that fetches 15 months of historical data when adding CD Portfolio companies. Features automatic domain extraction, historical data retrieval (Bounce Rate, Session Duration, Pages per Session, Sessions per User, Traffic Channels, Device Distribution), intelligent data processing and database mapping, portfolio average calculations across all companies, and complete data isolation from client/benchmark/competitor data. Integration runs in background during company creation to avoid UI blocking. Package includes modular service architecture (`server/services/semrush/`) with SEMrushService, SemrushDataProcessor, and PortfolioIntegration components.
- **SEMrush API v3 Integration Fix (August 2025)**: Resolved critical SEMrush API connectivity issues through comprehensive API testing with real domains (Amazon.com, GitHub.com). Fixed parameter naming (`targets` vs `target`), corrected column mappings (`time_on_site` vs `visit_duration`), and consolidated all data fetching to working `/summary` endpoint. Successfully validated API returns authentic data: Amazon shows 2.5B visits, 42.46% bounce rate, 693s session duration. All 6 target metrics now properly mapped and functional with production SEMrush API key.
- **Unified 2-Device Model Implementation (August 2025)**: Standardized device distribution across GA4 and SEMrush data sources to Desktop + Mobile format. Updated GA4DataProcessor to combine mobile and tablet traffic into single Mobile category, modified SEMrush integration to match this 2-device model, and updated all dashboard charts and components (LollipopChart, device color constants) to handle the simplified device structure. This aligns with SEMrush's data model where tablets are included in mobile traffic, ensuring consistent device reporting across all data sources.
