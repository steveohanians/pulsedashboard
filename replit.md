...
- **"With Context" Badge System**: Implemented comprehensive "With Context" badge display system for AI insights, using server-computed `hasContext` field. **Fixed critical data synchronization bug** (August 2025) where badges flashed and disappeared due to unsynchronized data sources between preloadedInsight and useAIInsights hook. **Fixed field name inconsistency bug** (August 2025) where two boolean fields (`hasContext` and `hasCustomContext`) were being maintained but only one was read, causing badges to not show when context was provided. Unified all code paths to use `hasContext` consistently with explicit boolean values.
