...
2. [PROMPTED] Question here`,
  ...
  // Effectiveness Insights Generation
  app.post("/api/effectiveness/insights/:clientId/:runId", requireAuth, async (req, res) => {
    try {
      ...
      const { effectivenessInsightsService } = await import('./services/effectiveness/insightsService');
      const insights = await effectivenessInsightsService.generateInsights({
        clientName: client.name,
        websiteUrl: client.websiteUrl,
