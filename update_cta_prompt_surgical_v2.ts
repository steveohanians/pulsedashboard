...
`;
  ...
  * If visual OCR is blank/degraded but text content contains CTAs in early positions (first 1500 chars), trust the text content for positional determination.`
    ...
    const updated = await storage.updateEffectivenessPromptTemplate('ctas', {
      promptTemplate: updatedTemplate,
      systemPrompt: currentPrompt.systemPrompt, // Keep existing
      schema: currentPrompt.schema // Keep existing
    });
    ...
    if (updated) {
      ...
      console.log('     • Above-the-fold: 250 chars → 1500 chars (35% of lines)');
      console.log('     • Added navigation recognition rules');
      console.log('     • Enhanced visual/text reconciliation');
      console.log('     • Added "let\'s chat" variants');
      
      // Verify the update
      const verification = await storage.getEffectivenessPromptTemplate('ctas');
      if (verification && verification.promptTemplate.includes('1500 characters')) {
        console.log('\n✓ Verification: Changes confirmed in database');
      }
    } else {
      console.log('❌ Update failed - no prompt updated');
    }
