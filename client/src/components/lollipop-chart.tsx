        ...
        <div className="relative h-full">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-24 flex flex-col justify-between py-4">
            {chartEntities.map((entity, index) => (
              <div key={index} className="text-sm text-gray-700 text-right pr-2 flex items-center justify-end h-12">
                <span className={entity.type === 'client' ? 'font-medium text-primary' : ''}>
                  {entity.label}
                </span>
              </div>
            ))}
              ...
              {chartEntities.map((entity, entityIndex) => (
                <div key={entityIndex} className="relative h-12 flex items-center">
                  {devices.map((device, deviceIndex) => {
                    ...
                    return (
                      <div key={device} className="absolute w-full" style={{ top: `${deviceIndex * 3}px` }}>
                        {/* Lollipop stick */}
                        <div
                          className="h-0.5"
                          style={{
